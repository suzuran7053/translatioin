...
class Category {
    ...
    // This function will be used only to get all categories. $sql is defined here.
    public static function find_all_posts(){
        return self::find_this_query("SELECT * FROM category");
    }

    // This function will be userd only to get a category by its id. $sql is defined here.
    public static function find_category_by_id($id){
        global $database;
        $the_result_array = self::find_this_query("SELECT * FROM category WHERE id = $id LIMIT 1");
